 ...
 * @LastEditTime: 2020-04-25 16:56:21
...
public:
    ...
    void insert( Comparable x ){
        if( root == nullptr ){
            root = new Node( x, NIL, NIL, nullptr, BLACK, 1 );
        }else{
            insert( root, x );
        }
    }
...
private:
    struct Node{
        ...
        Node() : value{ 0 }, color{ RED }, leftTree{ nullptr }, 
        rightTree{ nullptr }, parent{ nullptr }, size{ 0 } { }
        
        Node( Comparable val, Node *lf, Node *rt, Node *pt, bool col = RED, int s = 1 )
        : value{ val }, leftTree{ lf }, rightTree{ rt }, parent{ pt }, color{ col }, size{ s } { }
    ...
    void rotate_right( Node *p ){
        ...
        if( gp != nullptr )
            gp->size = size( gp );
        // fa->size = size( fa->rightTree ) + size( fa->leftTree ) + 1;
        // p->size = size( p->rightTree ) + size( p->leftTree ) + 1;
        // if( gp != nullptr )
        //     gp->size = size( gp->rightTree ) + size( gp->leftTree ) + 1;
    }
    // 左旋 p 的父节点 fa 
    ...
    void inorder( Node * p ){
        ...
        std::cout << p->value << "  " << p->size << std::endl;
    ...
    // 返回 p 子树的大小
    inline int size( Node *p ) const{
        int size = 0;
        if ( p != NIL ){
            if( p->leftTree != NIL )
                size += p->leftTree->size;
            if( p->rightTree != NIL )
                size += p->rightTree->size;
            return  ++size;
        }           
        return size;
        // return p->leftTree->size + p->rightTree->size + 1;
    }
    // 在以x为根的子树中取第i小的元素
    Comparable select( Node *p, int i ) const{
        int k = size( p->leftTree ) + 1;
        if( k == i )
            return p->value;
        else if( k > i )
            return select( p->leftTree, i );
        else
            return select( p->rightTree, i - k );
    }
};
